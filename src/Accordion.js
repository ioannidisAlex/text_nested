...
function Accordion() {
	...
	return (
		<div className={`accordion ${isAccordionOpen ? 'active' : ''}`}>
			...
			<div className={`accordion-content ${isAccordionOpen ? 'slide-up' : ''}`}>Content for Section 1</div>
			<div className="accordion-header" onClick={toggleAccordion}>
				Section 2
			</div>
			<div className={`accordion-content ${isAccordionOpen ? 'slide-up' : ''}`}>Content for Section 2</div>
		</div>
	);
}
